...
export default function MealFormModal({ open, onClose, onSave, meal, saving }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!validate()) return;
    // The actual upload to Supabase Storage happens inside vendorService
    // (addMeal/updateMeal) — we just hand over the raw File. `form.image`
    // is only used as a fallback when editing a meal and no new file was
    // chosen, so the existing photo is kept.
    onSave({
      ...form,
