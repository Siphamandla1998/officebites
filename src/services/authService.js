...
export const authService = {
  ...
  /**
   * Sends a password-reset email via Supabase Auth. One flow shared by
   * customer/vendor/admin — role has no bearing on who's allowed to reset a
   * password, only on what they can access once signed back in.
   * redirectTo must be an allowed Redirect URL in Supabase Auth settings
   * (Authentication → URL Configuration) or Supabase will reject the link.
   */
   ...
   * valid once Supabase has exchanged the reset-link token for a temporary
   * recovery session — see ResetPassword.jsx, which waits for the PASSWORD_RECOVERY
   * auth event before showing this form.
   */
