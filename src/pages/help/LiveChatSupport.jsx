...
export default function LiveChatSupport() {
  ...
  useEffect(() => {
    chatService.getSupportConversation().then((convo) => setMessages(convo.messages));
  }, []);
  ...
  const send = async (overrideText) => {
    ...
    const message = await chatService.sendSupportMessage(value);
    setMessages((prev) => [...prev, { ...message, status: "sent" }]);
    setText("");

    setTyping(true);
    const reply = await chatService.sendSupportAgentReply();
    setTyping(false);
