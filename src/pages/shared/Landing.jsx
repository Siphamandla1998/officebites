...
import { FiArrowRight, FiShield, FiTrendingUp, FiClock } from "react-icons/fi";
    ...
    <div className="min-h-screen flex flex-col">
      <div className="relative px-6 pt-14 pb-10 overflow-hidden">
        ...
        <div className="relative">
          <span className="inline-flex h-12 w-12 rounded-2xl bg-ink text-paper items-center justify-center text-base font-bold mb-6">
            OB
          </span>
          ...
          <p className="text-sm text-ink-muted mt-3 leading-relaxed">
            {APP_NAME} connects office teams with local food vendors — real meals, verified
            payments, and a ticket the moment your order is confirmed.
          </p>
      ...
      <div className="px-6 flex flex-col gap-4 pb-14">
        {PILLARS.map((p) => (
          <div key={p.title} className="card p-4 flex gap-3.5">
            <div className="h-10 w-10 rounded-xl bg-nude-100 text-nude-700 flex items-center justify-center shrink-0">
              <p.icon size={17} />
            </div>
            <div>
              <h3 className="text-sm font-semibold text-ink">{p.title}</h3>
              <p className="text-xs text-ink-muted mt-0.5 leading-relaxed">{p.desc}</p>
            </div>
          </div>
        ))}
      </div>
