...
export default function AdminPayments() {
  ...
  useEffect(() => {
    ...
    const loadProofUrls = async () => {
      const entries = await Promise.all((pending || []).filter((o) => o.paymentProof).map(async (o) => {
        try {
          return [o.id, await paymentService.getProofUrl(o.paymentProof)];
        } catch {
          return [o.id, null];
        }
      }));
      if (!cancelled) setProofUrls(Object.fromEntries(entries));
    };
    loadProofUrls();
        ...
        {selected && (
          <div className="flex flex-col gap-4">
            ...
            {selected.paymentProof ? (
              <img src={proofUrls[selected.id] || ""} alt="Proof of payment" className="w-full rounded-xl object-cover" />
            ) : (
