    ...
    <div>
      ...
      <div className="ob-container pt-1 pb-5">
        {isAuthenticated ? (
          ...
          <p className="text-xs text-ink-muted mb-1">Order from local office vendors — no account needed</p>
        )}
        <h2 className="text-xl font-bold text-ink tracking-tight">
          {isAuthenticated ? `Hey ${user.name.split(" ")[0]}, hungry?` : `Welcome to ${APP_NAME}`}
        </h2>

        {!isAuthenticated && (
          <p className="text-xs text-ink-muted mt-1.5">
            Browse and order freely —{" "}
            <Link to="/login" className="text-nude-600 font-medium">sign in</Link> anytime to save
            favourites and track orders across visits.
          </p>
        )}
