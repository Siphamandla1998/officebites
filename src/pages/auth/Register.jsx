...
export default function Register() {
  const [form, setForm] = useState({ name: "", email: "", password: "", role: ROLES.CUSTOMER, building: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const { user, needsEmailConfirmation } = await register(form);
      if (needsEmailConfirmation) {
        setConfirmSent(true);
        return;
      }
      showToast("Account created — welcome to OfficeBites!", { type: "success" });
      navigate(user.role === ROLES.VENDOR ? "/vendor" : user.role === ROLES.ADMIN ? "/admin" : "/");
    } catch (err) {
    ...
    return (
      <div className="card p-6 text-center flex flex-col gap-2">
        ...
        <p className="text-sm text-ink-muted">
          We've sent a confirmation link to <span className="font-medium text-ink">{form.email}</span>.
          Click it, then come back and sign in.
        </p>
        <Link to="/login" className="btn-primary mt-3">Go to sign in</Link>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {form.role === ROLES.VENDOR && (
          <p className="text-xs text-ink-muted bg-nude-50 rounded-lg px-3 py-2.5">
            Vendor accounts are reviewed by OfficeBites admin before your store goes live — an admin
            will set up your storefront and link it to this account.
          </p>
        )}
